...
# what are the dimentions of the filtered and unfiltered matrices?
dim(cpm_mat)
dim(cpm_mat_filt)
```
...
*Results columns* 
  ...
  * `padj`: The p-value adjusted for multiple comparisons using the Benjamini-Hochberg method. Also called the false discovery rate (FDR). Filtering for padj of `0.01` implies that 1% of significant tests will result in false positives. For a detailed explaination see StatQuest's video [False Discovery Rates, FDR, clearly explained](https://www.youtube.com/watch?v=K8LQSvtjcEo&ab_channel=StatQuestwithJoshStarmer)   
...
write_delim(resdf, file = paste(part2_results, "deseq2_results.txt", sep = "/")) 
